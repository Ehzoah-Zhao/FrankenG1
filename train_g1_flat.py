...
def save_masked_stats(dataset, run_dir):
    ...
    all_motion_data, all_motion_masks = [], []
    ...
    std_motionfeats = torch.sqrt(sum_squared_diff / divisor)
...
def train(cfg: DictConfig):
    ...
    if cfg.resume_dir is not None:
        ...
        logger.info("The config is loaded from: " + resume_dir)
    else:
        resume_dir = None
    ...
    train_dataloader = instantiate(cfg.dataloader, dataset=train_dataset,
        collate_fn=train_dataset.collate_fn, shuffle=True)
    val_dataloader = instantiate(cfg.dataloader, dataset=val_dataset,
