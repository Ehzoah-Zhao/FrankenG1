...
class MeshViewer(object):
    ...
    def animate(self, fps=30, start=None, end=None, progress_bar=tqdm, action_colorizer=None, title=None):
        """
        Modified animate method to support text annotations.
        
        Args:
            fps: Frames per second
            start: Start frame index
            end: End frame index
            progress_bar: Progress callback for rendering
            action_colorizer: ActionColorizer instance with annotations
            title: Optional title to display at the top of the rendered video
        """
        if not self.use_offscreen:
            print("=================================")
        ...
        # Store action_colorizer for rendering
        self.action_colorizer = action_colorizer
        self.fps = fps  # Store fps for time calculations
        ...
        # Define the range of frames to render
        actual_start = 0 if start is None else start
        actual_end = self.animation_len if end is None else end
        
        # Create iterator with progress bar if provided
        iterator = range(actual_start, actual_end)
        if progress_bar is not None:
            iterator = progress_bar(list(iterator), desc="SMPL rendering")

        for frame_idx in iterator:
            self.animation_frame_idx = frame_idx
            self.current_frame_idx = frame_idx  # Store current frame for text rendering
            ...
            if self.animation_frame_idx + 1 >= actual_end:
    ...
    # Update the render method to use the PIL-based text overlay
    def render(self):
        """Modified render method to add text annotations using PIL."""
        ...
        # Add text annotations if we have an action_colorizer and frame index
        if hasattr(self, 'action_colorizer') and hasattr(self, 'current_frame_idx'):
            # output_img = self.add_text_overlay_pil(
            #     output_img, 
            #     self.action_colorizer, 
            #     self.current_frame_idx
            # )
            output_img = self.add_text_info_bar(
                output_img, 
                self.action_colorizer, 
                self.current_frame_idx
            )
    ...
    def wrap_title_text(self, draw, title, font, max_width):
        """
        Wrap title text to fit within specified width with NO truncation.
        All text will be preserved across as many lines as needed.
        """
        words = title.split()
        lines = []
        current_line = ""
        ...
        for word in words:
            ...
            if text_width <= max_width:
                ...
                if current_line:
                    ...
                    # Single word is longer than max_width, but we still add it
                    # (no truncation - let it overflow if necessary)
                    lines.append(word)
                    current_line = ""
    ...
    def add_text_info_bar(self, img, action_colorizer, time_idx):
        """
        Add a clean annotation area with text labels in matching colors.
        Uses PIL for text rendering without showing confidence values.
        
        Args:
            img: The image to add annotations to
            action_colorizer: ActionColorizer instance
            time_idx: Current time index for annotation lookup
                
        Returns:
            Image with text annotation area
        """
        import numpy as np
        from PIL import Image, ImageDraw, ImageFont
        
        # Skip if no action colorizer
        if action_colorizer is None and not hasattr(self, 'title'):
            return img
        ...
        try:
            ...
            if len(img.shape) == 2:  # Grayscale
                ...
                pil_img = pil_img.convert('RGB')  # Convert to RGB for annotation
            ...
            # # Draw title at the top if it exists
            # if title is not None:
            ...
            #     try:
            #         title_font = ImageFont.truetype("/usr/share/fonts/dejavu/DejaVuSans.ttf", 12)  # Larger font for title
            #     except IOError:
            ...
            if title is not None:
                ...
                # Try to load a font, or use default
                try:
                    title_font = ImageFont.truetype("/usr/share/fonts/dejavu/DejaVuSans.ttf", 30)
                except IOError:
                    try:
                        # Fallback to Mono
                        title_font = ImageFont.truetype("/usr/share/fonts/dejavu/DejaVuSansMono.ttf", 30)
                    except IOError:
                        # Final fallback: default bitmap font (fixed small size)
                        title_font = ImageFont.load_default()
                
                # Calculate maximum width (leave some padding on sides)
                max_title_width = w - 40  # 20px padding on each side
                
                # Wrap the title text (no truncation)
                title_lines = self.wrap_title_text(draw, title, title_font, max_title_width)
                
                # Calculate dimensions
                line_height = title_font.size + 4  # Font size + spacing between lines
                total_text_height = len(title_lines) * line_height
                
                # Starting position (left-aligned with padding)
                # title_x = 20
                title_x = w // 2 # 居中
                title_y = 5
                ...
                # Draw background rectangle for all title lines
                padding = 5
                # bg_x1 = title_x - padding
                bg_x1 = title_x - max_line_width//2 - padding  # 居中对齐
                bg_y1 = title_y - padding
                # bg_x2 = title_x + max_line_width + padding
                bg_x2 = title_x + max_line_width//2 + padding  # 居中对齐
                bg_y2 = title_y + total_text_height + padding
                
                draw.rectangle(
                    [(bg_x1, bg_y1), (bg_x2, bg_y2)],
                    # fill=(0, 0, 0, 180)  # Semi-transparent black background
                    fill=(255, 255, 255, 230)  # Semi-transparent black background
                )
                
                # Draw each line of the title
                current_y = title_y
                for line in title_lines:
                    # draw.text((title_x, current_y), line, fill=(255, 255, 255), font=title_font)
                    # 每行居中
                    line_bbox = draw.textbbox((0, 0), line, font=title_font)
                    line_width = line_bbox[2] - line_bbox[0]
                    line_x = title_x - line_width // 2
                    draw.text((line_x, current_y), line, fill=(0, 0, 0), font=title_font)  # 黑色文字
                    
                    current_y += line_height            
            
            # Skip the rest if no action colorizer
            if action_colorizer is None:
                return np.array(pil_img)
            
            # Create a new image with space for annotations
            annotation_height = 200  # Reduced height since we're not showing confidence
            new_height = pil_img.height + annotation_height
            new_img = Image.new(pil_img.mode, (pil_img.width, new_height), (255, 255, 255))
            ...
            line_y = pil_img.height
            draw.line([(0, line_y), (pil_img.width, line_y)], fill=(0, 0, 0), width=2)
            
            # Try to load a font, or use default
            try:
                font = ImageFont.truetype("/usr/share/fonts/dejavu/DejaVuSans.ttf", 20)  # Slightly larger font
            except IOError:
                font = ImageFont.load_default()
            
            # Body parts to annotate
            body_parts = ["left_arm", "right_arm", "left_leg", "right_leg", "spine", "trajectory"]
            
            # Divide into two columns
            col_width = w // 2
            
            max_text_width = col_width - 40
            def draw_multiline_text(draw, pos, text, font, fill, max_width):
                words = text.split()
                line = ""
                y = pos[1]
                for word in words:
                    test_line = line + (" " if line else "") + word
                    bbox = draw.textbbox((pos[0], y), test_line, font=font)
                    if bbox[2] - bbox[0] > max_width:  # 超出宽度 -> 换行
                ...
                if line:
                    draw.text((pos[0], y), line, font=font, fill=fill)
            # # Left column body parts
            # left_parts = ["left_arm", "left_leg", "spine"]
            # # Right column body parts
            # right_parts = ["right_arm", "right_leg", "trajectory"]
            has_head = action_colorizer.get_annotation_at_time("head", time_idx) is not None
            if has_head:
                body_parts = ["left_arm", "right_arm", "left_leg", "right_leg", "spine", "head", "trajectory"]
                left_parts = ["left_arm", "left_leg", "spine", "head"]
                right_parts = ["right_arm", "right_leg", "trajectory", "action"]
            else:
                body_parts = ["left_arm", "right_arm", "left_leg", "right_leg", "spine", "trajectory"]
                left_parts = ["left_arm", "left_leg", "spine"]
                right_parts = ["right_arm", "right_leg", "trajectory", "action"]
            
            # Draw left column annotations
            for i, body_part in enumerate(right_parts):
                ann = action_colorizer.get_annotation_at_time(body_part, time_idx)
                if ann and "text" in ann and ann["text"] != "unknown":
                    action_text = ann["text"]
                    
                    # Calculate position
                    y_pos = pil_img.height + 5 + i * 40
                    x_pos = 5
                    
                    # Get color for this action
                    color = action_colorizer.action_colors.get(action_text, (0.7, 0.7, 0.7))
                    color_rgb = tuple(int(c * 255) for c in color)
                    
                    # Draw colored square
                    draw.rectangle([(x_pos, y_pos), (x_pos + 10, y_pos + 20)], fill=color_rgb)
                    
                    # Draw text with matching color
                    display_text = f"{body_part}: {action_text}"
                    # draw.text((x_pos + 20, y_pos), display_text, fill=color_rgb, font=font)
                    draw_multiline_text(draw, (x_pos + 20, y_pos), display_text, font, color_rgb, max_text_width)
            
            # Draw right column annotations
            for i, body_part in enumerate(left_parts):
                ann = action_colorizer.get_annotation_at_time(body_part, time_idx)
                if ann and "text" in ann and ann["text"] != "unknown":
                    action_text = ann["text"]
            ...
            result_img = np.array(new_img)
            
            return result_img
            
        except Exception as e:
            import traceback
            print(f"Error adding text annotations: {str(e)}")
            traceback.print_exc()
            # fallback: return image with white padding to keep size consistent
            h, w = img.shape[:2]
            result = np.ones((h+160, w, 3), dtype=np.uint8) * 255
            result[:h, :w, :3] = img[:, :, :3]
            return result

    def add_text_overlay_pil(self, img, action_colorizer, time_idx):
        """
        Add text annotations using PIL instead of OpenCV.
        This approach should work with various image formats.
        ...
        try:
            ...
            if img.dtype != np.uint8:
                img_arr = np.uint8(img * 255)
            else:
                img_arr = img
                
            # Convert numpy array to PIL Image
            if img_arr.shape[2] == 4:  # RGBA format
                pil_img = Image.fromarray(img_arr, 'RGBA')
            else:  # RGB format
                pil_img = Image.fromarray(img_arr[:,:,:3], 'RGB')
            ...
            # First pass: create legend at the top
            legend_y = 30
            legend_x = 50
            legend_spacing = 200
            current_x = legend_x
            
            # Track already displayed actions to avoid duplicates in the legend
            displayed_actions = set()
            
            for body_part in body_parts:
                ann = action_colorizer.get_annotation_at_time(body_part, time_idx)
                
                if ann and "text" in ann:
                    action_text = ann["text"]
                    
                    # Skip if we've already displayed this action in the legend
                    if action_text in displayed_actions or action_text == "unknown":
                        continue
                        
                    displayed_actions.add(action_text)
                    ...
                    # Draw color square
                    draw.rectangle(
                        [(current_x, legend_y-15), (current_x+15, legend_y)], 
                        fill=color_rgb
                    )
                    
                    # Draw text
                    draw.text(
                        (current_x + 20, legend_y - 15), 
                        action_text, 
                        fill=color_rgb, 
                        font=font
                    )
            ...
            # Define positions for each body part's text
            positions = {
                "left_arm": (int(w * 0.1), int(h * 0.2)),
                "right_arm": (int(w * 0.8), int(h * 0.2)),
                "left_leg": (int(w * 0.15), int(h * 0.7)),
                "right_leg": (int(w * 0.75), int(h * 0.7)),
                "spine": (int(w * 0.5), int(h * 0.1)),
                "trajectory": (int(w * 0.5), int(h * 0.9))
            }
            
            # Second pass: add text near each body part
            for body_part in body_parts:
                ann = action_colorizer.get_annotation_at_time(body_part, time_idx)
                ...
                if ann and "text" in ann and ann["text"] != "unknown":
                    ...
                    # Get position for this body part
                    pos_x, pos_y = positions[body_part]
                    ...
                    text_bbox = draw.textbbox((pos_x, pos_y), display_text, font=font)
                    draw.rectangle(
                        [
            ...
            import traceback
            print(f"Error adding text overlay with PIL: {str(e)}")
            traceback.print_exc()
            return img