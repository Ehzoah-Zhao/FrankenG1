...
class TimestepEmbedder(nn.Module):
    def __init__(self, dim, sequence_pos_encoder=None, use_fourier=False):
        super().__init__()
        self.dim = dim
        ...
        if use_fourier:
            self.embedder = FourierEmbedding(dim)
        else:
            if sequence_pos_encoder is not None:
                # Original implementation using pos encoder
                time_embed_dim = self.dim
                self.time_embed = nn.Sequential(
                    nn.Linear(self.dim, time_embed_dim),
                    nn.SiLU(),
                    nn.Linear(time_embed_dim, time_embed_dim),
                )
            else:
                # DFOT style implementation
    ...
    def forward(self, timesteps):
        if self.use_fourier:
            return self.time_embed(self.embedder(timesteps))
        
        if self.sequence_pos_encoder is not None:
            # Original implementation
            return self.time_embed(self.sequence_pos_encoder.pe[timesteps.int()][..., 0, :])
        else:
            # DFOT style implementation
            return self.time_embed(self.timesteps(timesteps))
