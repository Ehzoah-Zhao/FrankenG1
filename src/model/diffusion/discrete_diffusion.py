...
from .diffusion_base import DiffuserBase, extract  # Import DiffuserBase and extract from base.py
...
class DiscreteDiffusion(DiffuserBase):
    def __init__(
        self,
        cfg: DictConfig,
        model: nn.Module,
        ...
        self.schedule_fn_kwargs = getattr(cfg, "schedule_fn_kwargs", {}) ###need to change###
        self.prediction = cfg.prediction
        self.loss_weighting = cfg.loss_weighting
        self.ddim_sampling_eta = getattr(cfg, "ddim_sampling_eta", 0.0)
        self.clip_noise = getattr(cfg, "clip_noise", 20.0)
        self.use_causal_mask = getattr(cfg, "use_causal_mask", False)
        
        # If a custom schedule is provided, use that, otherwise create one from cfg
        if custom_schedule is not None:
            schedule_fn = custom_schedule
        else:
            schedule_fn = lambda timesteps: make_beta_schedule(
                schedule=self.beta_schedule,
                timesteps=timesteps,
                zero_terminal_snr=self.prediction != "noise",
                **self.schedule_fn_kwargs,
            )
        
        # Initialize DiffuserBase with the schedule
        super().__init__(schedule_fn, self.timesteps)

        self.model = model
        # Additional buffers specific to DiscreteDiffusion
        self._build_additional_buffers()

    def _build_additional_buffers(self):
        """
        Build additional buffers specific to DiscreteDiffusion beyond those 
        created by DiffuserBase.
        """
        ...
        # Add loss weighting buffers
        if hasattr(self.loss_weighting, "strategy"):
    ...
    def add_shape_channels(self, x):
        """Add extra dimensions to match the tensor shape."""
        if hasattr(self, "x_shape") and self.x_shape is not None:
            return rearrange(x, f"... -> ...{' 1' * len(self.x_shape)}")
        else:
            # Default behavior if x_shape is not defined
    ...
    def model_predictions(self, x, k, external_cond=None, external_cond_mask=None):
        ...
        model_output = self.model(x, external_cond, k)
    ...
    # Override q_sample from DiffuserBase
    ...
    def compute_loss_weights(
        ...
        if strategy == "sigmoid":
            ...
            sigmoid_bias = getattr(self.loss_weighting, "sigmoid_bias", 5.0)
            epsilon_weighting = torch.sigmoid(sigmoid_bias - logsnr)
            ...
            snr_clip = getattr(self.loss_weighting, "snr_clip", 5.0)
            cum_snr_decay = getattr(self.loss_weighting, "cum_snr_decay", 0.5)
            clipped_snr = self.clipped_snr[k]
    ...
    def ddim_sample_step(
        ...
        clipped_curr_noise_level = torch.clamp(curr_noise_level, min=0)
        print(clipped_curr_noise_level[0])
