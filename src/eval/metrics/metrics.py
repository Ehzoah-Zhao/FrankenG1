...
def calculate_jerk(poses, lengths):
    """
    ...
    mask = np.zeros(jerk.shape[0:2], dtype=bool)
...
def calculate_top_k(mat, top_k, gt_indices=None):
    possibilities_size = mat.shape[0]
    sample_size = mat.shape[1]

    if gt_indices is None:
        assert possibilities_size == sample_size
        gt_indices = np.arange(sample_size)
    gt_mat = gt_indices[None].repeat(possibilities_size, axis=0)  # shape (possibilities, samples)
    bool_mat = (mat == gt_mat)
    correct_vec = False
    top_k_list = []
...
def calculate_diversity_normalized(activation, diversity_times):
    assert len(activation.shape) == 2
    assert activation.shape[0] > diversity_times
    num_samples = activation.shape[0]

    activation = activation / np.linalg.norm(activation, axis=-1)[:, None]
